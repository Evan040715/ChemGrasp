"""
Tabletop Isaac entry: same interface as isaac_main.py but uses IsaacValidatorTabletop
(object on table, gravity on, ground plane). For desktop/tabletop grasping validation.
"""
import os
import sys
import json
import argparse
import warnings
from termcolor import cprint

ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(ROOT_DIR)

from validation.isaac_validator_tabletop import IsaacValidatorTabletop
from utils.hand_model import create_hand_model
from utils.rotation import q_rot6d_to_q_euler

import torch


def isaac_main_tabletop(
    mode: str,
    robot_name: str,
    object_name: str,
    batch_size: int,
    q_batch: torch.Tensor = None,
    gpu: int = 0,
    use_gui: bool = False,
    table_height: float = 0.75,
    object_center_to_bottom_z: float = 0.03,
    table_size: float = 0.75,
):
    """
    Tabletop validation: object on table, gravity on, ground plane.

    :param mode: str, 'filter' or 'validation'
    :param robot_name: str
    :param object_name: str
    :param batch_size: int
    :param q_batch: torch.Tensor (validation only)
    :param gpu: int
    :param use_gui: bool
    :param table_height: float, table surface z (ground at 0)
    :param object_center_to_bottom_z: float, height from object center to bottom (m), so object sits on table
    :param table_size: float, table top surface size in m (e.g. 0.75 = 0.75m x 0.75m)
    :return: success, q_isaac (same as isaac_main)
    """
    if mode == 'filter' and batch_size == 0:
        return 0, None
    if use_gui:
        gpu = 0

    data_urdf_path = os.path.join(ROOT_DIR, 'data/data_urdf')
    urdf_assets_meta = json.load(open(os.path.join(data_urdf_path, 'robot/urdf_assets_meta.json')))
    robot_urdf_path = urdf_assets_meta['urdf_path'][robot_name]
    object_name_split = object_name.split('+') if object_name is not None else None
    object_urdf_path = f'{object_name_split[0]}/{object_name_split[1]}/coacd_decomposed_object_one_link.urdf'

    hand = create_hand_model(robot_name)
    joint_orders = hand.get_joint_orders()

    simulator = IsaacValidatorTabletop(
        robot_name=robot_name,
        joint_orders=joint_orders,
        batch_size=batch_size,
        gpu=gpu,
        is_filter=(mode == 'filter'),
        use_gui=use_gui,
        table_height=table_height,
        object_center_to_bottom_z=object_center_to_bottom_z,
        table_size=table_size,
    )
    print("[Isaac Tabletop] IsaacValidatorTabletop is created.")

    simulator.set_asset(
        robot_path=os.path.join(data_urdf_path, 'robot'),
        robot_file=robot_urdf_path[21:],
        object_path=os.path.join(data_urdf_path, 'object'),
        object_file=object_urdf_path
    )
    simulator.create_envs()
    print("[Isaac Tabletop] Preparation is done.")

    if mode == 'filter':
        dataset_path = os.path.join(ROOT_DIR, 'data/CMapDataset/cmap_dataset.pt')
        metadata = torch.load(dataset_path)['metadata']
        q_batch = [m[1] for m in metadata if m[2] == object_name and m[3] == robot_name]
        q_batch = torch.stack(q_batch, dim=0).to(torch.device('cpu'))
    if q_batch.shape[-1] != len(joint_orders):
        q_batch = q_rot6d_to_q_euler(q_batch)

    simulator.set_actor_pose_dof(q_batch.to(torch.device('cpu')))
    success, q_isaac = simulator.run_sim()
    simulator.destroy()

    return success, q_isaac


if __name__ == '__main__':
    warnings.simplefilter(action='ignore', category=FutureWarning)
    parser = argparse.ArgumentParser()
    parser.add_argument('--mode', type=str, required=True)
    parser.add_argument('--robot_name', type=str, required=True)
    parser.add_argument('--object_name', type=str, required=True)
    parser.add_argument('--batch_size', type=int, required=True)
    parser.add_argument('--q_file', type=str)
    parser.add_argument('--gpu', default=0, type=int)
    parser.add_argument('--use_gui', action='store_true')
    parser.add_argument('--table_height', default=0.75, type=float, help='Table surface z (m)')
    parser.add_argument('--object_center_to_bottom_z', default=0.03, type=float, help='Height from object center to bottom (m)')
    parser.add_argument('--table_size', default=0.75, type=float, help='Table size in m (0.75 = 0.75m x 0.75m x 0.75m)')
    args = parser.parse_args()

    print(f'GPU: {args.gpu}, table_height: {args.table_height}, table_size: {args.table_size}, object_center_to_bottom_z: {args.object_center_to_bottom_z}')
    assert args.mode in ['filter', 'validation'], f"Unknown mode: {args.mode}!"
    q_batch = torch.load(args.q_file, map_location='cpu') if args.q_file is not None else None
    success, q_isaac = isaac_main_tabletop(
        mode=args.mode,
        robot_name=args.robot_name,
        object_name=args.object_name,
        batch_size=args.batch_size,
        q_batch=q_batch,
        gpu=args.gpu,
        use_gui=args.use_gui,
        table_height=args.table_height,
        object_center_to_bottom_z=args.object_center_to_bottom_z,
        table_size=args.table_size,
    )

    success_num = success.sum().item()
    if args.mode == 'filter':
        print(f"<{args.robot_name}/{args.object_name}> (tabletop) before: {args.batch_size}, after: {success_num}")
        if success_num > 0:
            q_filtered = q_isaac[success]
            save_dir = str(os.path.join(ROOT_DIR, 'data/CMapDataset_filtered_tabletop', args.robot_name))
            os.makedirs(save_dir, exist_ok=True)
            torch.save(q_filtered, os.path.join(save_dir, f'{args.object_name}_{success_num}.pt'))
    elif args.mode == 'validation':
        cprint(f"[{args.robot_name}/{args.object_name}] (tabletop) Result: {success_num}/{args.batch_size}", 'green')
        save_data = {
            'success': success,
            'q_isaac': q_isaac
        }
        os.makedirs(os.path.join(ROOT_DIR, 'tmp'), exist_ok=True)
        torch.save(save_data, os.path.join(ROOT_DIR, f'tmp/isaac_main_tabletop_ret_{args.gpu}.pt'))
